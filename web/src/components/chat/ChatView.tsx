...
export function ChatView({ onChanged }: { onChanged: () => void }) {
  ...
  async function send(text: string) {
    // 重入防护放在这里，而不是只靠调用方。原来只有 ChatComposer 一个入口，
    // 它自己判了 sending；现在冲突卡片也能直接调 send，漏判一处就会让两轮
    // 对话叠在一起——store 只认"最后一条消息"，后到的 text_delta 会追加到
    // 前一轮的气泡上，看起来像模型在胡言乱语。
    if (sending) return;
